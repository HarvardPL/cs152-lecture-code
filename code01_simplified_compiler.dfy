...
include "code01_simplified.dfy"

// Stack machine instructions
datatype instr = IPush(v: int) | ILoad(x: string) | IAdd
...
// Concrete example: 2 + (3 + 4) = 9
lemma {:timeLimit 50} Example()
{
    var e := EAdd(EInt(2), EAdd(EInt(3), EInt(4)));
    var env: string -> int := _ => 0;
    assert compile(e) == [IPush(2), IPush(3), IPush(4), IAdd, IAdd];
    CompilerCorrect(e, env);
    assert exec(compile(e), [], env) == Some([9]);
}
